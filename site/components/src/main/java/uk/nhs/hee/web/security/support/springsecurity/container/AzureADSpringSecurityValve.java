package uk.nhs.hee.web.security.support.springsecurity.container;
 ...
 * <P>
 * This is responsible for reading the <code>org.springframework.security.core.Authentication</code> instance
 * if exists, and establishing <code>javax.security.auth.Subject</code> for the whole HST-2 request processing
 ...
 * </P>
 */
public class AzureADSpringSecurityValve extends AbstractOrderableValve {

    private static Logger log = LoggerFactory.getLogger(AzureADSpringSecurityValve.class);
    ...
    public void invoke(ValveContext context) throws ContainerException {
        ...
        // If a subject has been established already (normally by HST-2's SecurityValve),
        // then simply move to the next valve chain.
        if (subject != null) {
            if (log.isDebugEnabled()) {
        ...
        if (authentication == null) {
            if (log.isWarnEnabled()) {
                log.warn("Spring Security hasn't establish security context with authentication object. " +
                        "Skipping SpringSecurityValve...");
            }
            context.invokeNext();
            return;
        }
        ...
        AzureADUser userDetails = (AzureADUser) springSecurityPrincipal;
        ...
        if (storeSubjectRepositoryCredentials) {
            ...
            // Note: password should be null by default from some moment after Spring Security
            // version upgraded a while ago. if password is null, let's store a dummy password instead.
        ...
        // Save the created subject as http session attribute which can be read
        // by HST-2 SecurityValve in the next valve chain.
        request.getSession(true).setAttribute(ContainerConstants.SUBJECT_ATTR_NAME, subject);

        log.debug("Adding accessToken to session");
        request.getSession(true).setAttribute("accessToken", userDetails.getAccessToken());
